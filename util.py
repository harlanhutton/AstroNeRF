...
def add_datetime(func):
    def wrapper(*args,**kwargs):
        ...
        print(grey("[{}] ".format(datetime_str),bold=True),end="",flush=True)
        return func(*args,**kwargs)
    return wrapper

def add_functionname(func):
    def wrapper(*args,**kwargs):
        print(grey("[{}] ".format(func.__name__),bold=True),flush=True)
        return func(*args,**kwargs)
    return wrapper
...
class Log:
    ...
    def process(self,pid):
        print(grey("Process ID: {}".format(pid),bold=True),flush=True)
    def title(self,message):
        print(yellow(message,bold=True,underline=True),flush=True)
    def info(self,message):
        print(magenta(message,bold=True),flush=True)
    def options(self,opt,level=0):
        for key,value in sorted(opt.items()):
            if isinstance(value,(dict,edict)):
                print("   "*level+cyan("* ")+green(key)+":",flush=True)
                self.options(value,level+1)
            else:
                print("   "*level+cyan("* ")+green(key)+":",yellow(value),flush=True)
    def loss_train(self,opt,it,lr,loss,timer):
        #if not opt.max_epoch: return
        message = grey("[train] ",bold=True)
        message += "iter {}/{}".format(cyan(it,bold=True),opt.max_iter)
    ...
    def loss(self,loss):
        message = grey("[gt loss] ",bold=True)
        message += "loss:{}".format(red("{:.3e}".format(loss),bold=True))
        print(message,flush=True)
log = Log()

def update_timer(opt,timer,it):
    #if not opt.max_epoch: return
    momentum = 0.99
    ...
    timer.arrival = timer.it_mean*(opt.max_iter-it)
...
def restore_checkpoint(opt,model,load_name=None,resume=False):
    ...
    if resume:
        ...
        print("resuming from epoch {0} (iteration {1})".format(ep,it),flush=True)
...
def uniquify(path,dr):
    counter = 1
    new_path = path + '/' + dr + str(counter)
